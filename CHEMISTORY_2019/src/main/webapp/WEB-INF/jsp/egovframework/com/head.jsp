...
<input name="siteType" type="hidden" value="ADMIN"/>
 <link rel="stylesheet" type="text/css" href="<c:url value='/css/egovframework/com/common.css' />" />
    ...
    <div id="use_descri">
            <ul>
                <li>공통서비스 테스트 사이트(업무사용자용) &nbsp;&nbsp;<a href="javascript:actionLogout()"><img src="<c:url value='/images/egovframework/com/logout_btn.gif' />" alt="로그아웃" /></a></li>
            </ul>
    </div>
    </div>
    <div id="new_topnavi">
    	...
    	<tr>
    		<c:forEach var="result" items="${list_headmenu}" varStatus="status">
			   <td><a href="javascript:fn_main_headPageMove('<c:out value="${result.menuNo}"/>','<c:out value="${result.chkURL}"/>')"><c:out value="${result.menuNm}"/></a></td>
			</c:forEach>
    	</tr>
    	</table>
    </div>
