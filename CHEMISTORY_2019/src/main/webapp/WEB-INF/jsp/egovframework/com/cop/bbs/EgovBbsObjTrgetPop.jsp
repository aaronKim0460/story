...
<script type="text/javascript">
	...
	function fn_egov_select_brdMstr(pageNo){
		...
		document.frm.action = "<c:url value='/cop/bbs/SelectBBSMasterInfs_trget.do'/>";
		document.frm.submit();
	}
	...
	function fn_egov_select_trgetBbsId(trgetId){
		var parentFrom = opener.document.getElementsByTagName('form');
		parentFrom[0].target_bbs_id.value = trgetId;
	    window.close();
	}
</script>
	...
	<table width="100%" cellpadding="8" class="table-search" border="0">
	 <tr>
	  <td width="40%" class="title_left">
	   <h1><img src="<c:url value='/images/egovframework/com/cmm/icon/tit_icon.gif' />" width="16" height="16" hspace="3" align="middle" alt="">&nbsp;게시물 관리</h1>
	  </td>
		<td width="*" >
	...
	<table width="100%" cellpadding="8" class="table-line" summary="번호,게시판명,게시판유형,게시판속성,생성일,사용여부  목록입니다" >
	 <thead>
	  <tr>
	    ...
	    <th scope="col" class="title" width="8%" nowrap>사이트구분</th>
	 ...
	 <tbody>
		 <c:forEach var="result" items="${resultList}" varStatus="status">
		  <tr>
		    ...
		    <td class="lt_text3" nowrap><c:out value="${result.codeNm }"/></td>
		    <td class="lt_text3" nowrap>
				...
				<a href="javascript:fn_egov_select_trgetBbsId('<c:out value='${result.bbsId }'/>')">
		    ...
		    <td class="lt_text3" nowrap><c:out value="${result.ntceDate}"/></td>
		    <td class="lt_text3" nowrap>
