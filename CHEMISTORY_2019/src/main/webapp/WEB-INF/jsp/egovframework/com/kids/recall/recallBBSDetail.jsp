...
<script type="text/javascript">
	...
	function fn_do_update() {
		...
		if (typeof $(":input[name='mIdx']") === 'undefind') {
			alert("원인물질(유해물질)을 하나 이상 선택해 주시기 바랍니다.");
			return;
		}		
		
		if (confirm('<spring:message code="common.update.msg" />')) {
			document.recall.action = "<c:url value='/com/kids/recall/recallBBSUpdate.do'/>";
...
<form id="recall" name="recall" enctype="multipart/form-data" method="post">
	...
	<input type="hidden" name="returnUrl" value="<c:url value='/com/kids/recall/recallBBSDetail.do'/>"/>
	<input type="hidden" id="delType" value="recall"/>
		...
		<table class="bbs_default write" border="1">
			...
			<tbody>
				<tr>
					...
					<td>
						<input name="productName" id="productName" type="text" value="<c:out value="${result.productName}" />" class="input-text full" title="제품명 입력">
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<input name="publishDt" id="publishDt" type="text" value="<c:out value="${result.publishDt}" />" 
							class="input-text full" title="공표일입력" onkeydown='return onlyNumber(event)' onkeyup='removeChar(event)'/>
				...
				<!-- <tr>
					...
					<td>
						<input type="radio" name="domesticYn" value="Y" <c:if test="${result.domesticYn eq 'Y'}"> checked="checked" </c:if>/> 국내 &nbsp;
						<input type="radio" name="domesticYn" value="N" <c:if test="${result.domesticYn eq 'N'}"> checked="checked" </c:if>/> 국외
					</td>
				</tr> -->
				...
				<tr id="spot">
					...
					<td>
						<select name="productTabCd" id="productTabCd" class="board-search-select" style="width:220px">
							<option value="" <c:if test="${result.productTabCd eq null || result.productTabCd eq ''}"> selected="selected" </c:if>>분류 선택</option>
							<option value="1100701" <c:if test="${result.productTabCd eq '1100701'}"> selected="selected" </c:if>>리콜명령</option>
							<option value="1100702" <c:if test="${result.productTabCd eq '1100702'}"> selected="selected" </c:if>>환경유해인자 기준초과</option>
						</select><br/>
					</td>
				</tr>
				</c:if>
				...
				<tr>
					...
					<td>
						<input name="productWhence" id="productWhence" type="text" value="<c:out value="${result.productWhence}" />" class="input-text" size="50" title="출처 입력">
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="">국가</label></th>
					<td>
						<input name="countryName" id="countryName" type="text" value="<c:out value="${result.countryName}" />" class="input-text" size="50" title="국가 입력">
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<div id="materialArea">
							<c:forEach var="row" items="${result.materialList}" varStatus="i">
								<div>
									<strong><c:out value="${row.materialName}" /></strong>
									&nbsp;<a href="javascript:void(0);" onclick="fn_delete_material(this);" class="btn-sm btn-warning">삭제</a>
									<input name="mIdxs" type="hidden" value="<c:out value="${row.mIdx}" />" />
								</div>
							</c:forEach>
						</div>
					</td>
				...
				<tr>
					...
					<td>
						<textarea id="productCn" name="productCn" class="textarea-board" cols="50" rows="2" title="제품설명 입력"><c:out value="${result.productCn}" /></textarea>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="">세부사항</label></th>
					<td>
						<textarea id="recallCn" name="recallCn" class="textarea-board" cols="50" rows="2" title="세부사항 입력"><c:out value="${result.recallCn}" /></textarea>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="">후속 조치 내용</th>
					<td>
						<input name="afterRecallCn" id="afterRecallCn" type="text" value="<c:out value="${result.afterRecallCn}" />" class="input-text full" title="후속 조치 내용 입력">
					</td>
				</tr>
				<c:choose>
