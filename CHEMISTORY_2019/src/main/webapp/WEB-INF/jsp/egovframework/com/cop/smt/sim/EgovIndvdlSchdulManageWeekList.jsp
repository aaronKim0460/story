...
<%@ page import="egovframework.com.cmm.service.EgovWebUtil"%>
...
int iNowYear = Integer.parseInt(EgovWebUtil.clearXSSMinimum((String)request.getAttribute("year")));
int iNowMonth = Integer.parseInt(EgovWebUtil.clearXSSMinimum((String)request.getAttribute("month")));
int iNowWeek = Integer.parseInt(EgovWebUtil.clearXSSMinimum((String)request.getAttribute("week")));
    ...
    <td>
     	<select name="schdulSe" class="select" id="schdulSe" onChange="fnEgovSchdulSe(document.forms[0].schdulSe.options[document.forms[0].schdulSe.selectedIndex].value);" title="일정구분선택">
			...
			<c:forEach var="result" items="${schdulSe}" varStatus="status">
				<option value='<c:out value='${result.code}'/>' <c:if test="${searchKeyword == result.code}">selected</c:if>><c:out value='${result.codeNm}'/></option>
			</c:forEach>
  		</select>
