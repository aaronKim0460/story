...
function linkPage(pageNo){
    ...
    document.listForm.action = "<c:url value='/adm/contents/material/addEnvMaterial.do'/>";
...
doInsertCheck = function(idx,kname){
	$("#idx").val(idx);
	$("#kName").val(kname);	
};

doInsertMapping = function(idx,kname,ename){
	var tempidx = idx;
	var tempkname = kname;
	var tempename = ename;
	
	opener.mappParm(tempidx,tempkname,tempename);
	window.close();
	/* $.ajax({
		type: "POST"
		,url: "<c:url value="/adm/contents/material/insertMatMapping.do"/>"
		,data:$("#listForm").serialize()
		,success: function(result){                
            if("exist" == result){
            	alert("중복 추가 입니다");
            }else if("success" == result){
            	opener.refresh(envId);
            	window.close();
            }else{
            	alert("추가에 실패하였습니다. \n 관리자에게 문의해주세요");
            }
	...
	<table class="board-default">
		...
		<tbody>			
			...
			<tr class="notPointer">
				...
				<td class="board-default-td-title"><a href="#none" onclick="doInsertMapping('${env.idx}','${env.kName}','${env.eName}')"><c:out value="선택"/></a></td>				
			</tr>
			</c:forEach>
