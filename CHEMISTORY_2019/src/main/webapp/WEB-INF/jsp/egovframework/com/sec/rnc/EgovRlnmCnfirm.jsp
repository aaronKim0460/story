...
function fnNextMove(){
	...
	if(document.cnfirmForm.result.value=="success.user.rlnmCnfirm") {
    	...
    	alert("<spring:message code="<c:out value='${result}'/>" />");
	}
...
<!-- content start -->
        ...
        <input type="hidden" name="sbscrbTy" value="<c:out value='${sbscrbTy}'/>">
        <!-- GPIN 실명인증 으로 가기위한 초기화값 -->
        <input type="hidden" name="realName" value="<c:out value='${realName}'/>"><!-- 주민등록확인의 realname 변수와 구별  -->
        <!-- 실명인증을 재귀적으로 호출하는 경우 다음경로와 다음경로 버튼명을 셋팅함 -->
        <input type="hidden" name="nextUrl" value="<c:out value='${nextUrl}'/>">
        <input type="hidden" name="nextUrlName" value="<c:out value='${nextUrlName}'/>">
        ...
        <table width="717" border="0"  cellspacing="0" cellpadding="0">
            <tr>
	            <td>
	                &nbsp;이름:<input type="text" name="realname" value="<c:out value='${realname}'/>" title="이름입력">
	                &nbsp;주민등록번호 <input type="text" name="ihidnum" value="<c:out value='${ihidnum}'/>" title="주민등록번호입력">(-없이 13자리)
                    <!-- 사용자유형 --><input type="hidden" name="sbscrbTy_view" value="<c:out value='${sbscrbTy}'/>">
                    <!-- 결과코드 -->
                    <input type="hidden" name="result" value="<c:out value='${result}'/>">
                    <input type="hidden" name="result_tmp" value="<c:out value='${result_tmp}'/>">
                    <br><c:if test="${not empty result}"><spring:message code="<c:out value='${result}'/>" /></c:if>
                    <br>
                    <br>
	            </td>
            </tr>
            ...
            <tr>
                ...
                <tr>
                    ...
                    <td align="left"><img src="<c:url value='/images/egovframework/com/cmm/btn/bu2_right.gif' />" width="8" height="20" alt="버튼이미지"></td>
                     ...
                     <td>&nbsp;</td>
                     <td><span class="button"><input type="button" value="<spring:message code="<c:out value='${nextUrlName}'/>" />" title="<spring:message code="<c:out value='${nextUrlName}'/>" />" onclick="javascript:fnNextMove();return false;"></span></td>
