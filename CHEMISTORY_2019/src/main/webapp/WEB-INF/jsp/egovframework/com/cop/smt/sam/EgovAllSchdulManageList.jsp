...
<table width="100%" cellpadding="1" class="table-search" border="0">
 <tr>
  ...
  <td width="180px">
    <input name="searchKeyword" type="text" size="10" value="<c:out value='${searchKeyword}'/>" maxlength="35" style="width:100%" title="검색어 입력">
  </td>
  <td></td>
  ...
  <tr>
		...
		<td class="lt_text3L">
	   		<form name="subForm" method="post" <c:if test="${resultInfo.schdulKindCode == '1'}">action="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDetail.do'/>"</c:if><c:if test="${resultInfo.schdulKindCode == '2'}">action="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageDetail.do'/>"</c:if> >
				<input name="schdulId" type="hidden" value="<c:out value='<c:out value='${resultInfo.schdulId}'/>'/>">
				<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>"/>
	    		<span class="link"><input type="submit" style="width:320px;border:solid 0px black;text-align:left;" value="<c:out value="${resultInfo.schdulNm}"/>" onclick="JavaScript:fn_egov_detail_AllSchdulManage('<c:out value='${resultInfo.schdulKindCode}'/>','<c:out value='<c:out value='${resultInfo.schdulId}'/>'/>'); return false;"></span>
	    	</form>
		</td>
    	<td class="lt_text3"><c:out value='${resultInfo.frstRegisterNm}'/></td>
    	<td class="lt_text3"><c:out value='${resultInfo.ntceDate}'/></td>
