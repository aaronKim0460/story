			...
			<tr>
				<td class="listLeft" width="70%" style="padding-left:10px;" nowrap>
		    		<c:choose>
		     		...
		     		<c:out value="${fn:replace(result.commentCn, bLine, '<br>')}" escapeXml="false" />
		    </td>
		    <td class="listLeft" style="padding-left:10px;" nowrap>

		    <c:if test="${anonymous == 'true' || result.wrterId == sessionUniqId}">
		     <a href="javascript:fn_egov_selectCommentForupdt('<c:out value="${result.commentNo}" />', '<c:out value="${status.index}" />');" class="btn seablue"><spring:message code="button.update" /></a>
		      | <a href="javascript:fn_egov_deleteCommentList('<c:out value="${result.commentNo}" />', '<c:out value="${status.index}" />');" class="btn orange"><spring:message code="button.delete" /></a>
		    </c:if>
		    <c:if test="${anonymous == 'true'}">
		    <br>
...
<table class="bbs_default write" border="1">
	<tr>
		<th scope="row">작성자<span title="필수입력">*</span></th>
		<td>
	...
	<tr>
		<th scope="row">내용</th>
	    <td>
	      <textarea name="commentCn" class="textarea"  cols="50" rows="2"  style="width:90%;" title="댓글내용입력"><c:out value="${searchVO.commentCn}" /></textarea>
	      <form:errors path="commentCn" />
	    </td>
	  </tr>
	  <c:if test="${anonymous == 'true'}">
		  <tr>
		    <th scope="row">패스워드<span title="필수입력">*</span></th>
		    <td colspan="3">
	...
	<c:choose>
		<c:when test="${searchVO.commentNo == ''}">
	      <a href="javascript:fn_egov_insert_commentList()" class="btn seablue">댓글등록</a>
		</c:when>
		<c:otherwise>
	      <a href="javascript:fn_egov_updt_commentList()" class="btn seablue">댓글수정</a>
		</c:otherwise>
	</c:choose>
      <a href="javascript:fn_egov_select_commentList('1')" class="btn orange">댓글초기화</a>
	</div>
