...
*/
	...
	if (!validateWordDicaryVO(form)) {
		...
		alert(document.getElementById('wordDc').value);
...
<tbody>	
  ...
  <tr>
		<td class="board-write-editor" colspan="2">
				<textarea id="wordDc" name="wordDc" class="textarea-board" cols="75" rows="20"><c:out value="${wordDicaryVO.wordDc}" escapeXml="false" /></textarea>
...
<div class="text-center">
	...
	<input type="submit" value="<spring:message code="button.update" />" onclick="fn_egov_updt_worddicary(document.forms[0],'<c:out value="${result.wordId}"/>'); return false;" class="btn btn-primary" />
</div>

<input name="wordId" id="wordId" type="hidden" value="<c:out value="${result.wordId}"/>">
