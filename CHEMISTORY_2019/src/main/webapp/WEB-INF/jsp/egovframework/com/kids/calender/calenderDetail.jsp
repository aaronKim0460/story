...
<script type="text/javascript">
	...
	function fn_do_update() {
		...
		if (confirm('<spring:message code="common.update.msg" />')) {
			document.calender.action = "<c:url value='/com/kids/calender/calenderUpdate.do'/>";
		...
		<table class="bbs_default write" border="1">
			...
			<tbody>
				<tr>
					...
					<td>
						<input name="title" id="title" type="text" value="<c:out value="${result.title}" />" class="input-text full" title="주제 입력">
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="">구분</label><span title="필수입력">*</span></th>
					<td><c:if test="${result.gubun eq ''}"> checked="checked"</c:if>
						<input type="radio" name="gubun" value="교육" <c:if test="${result.gubun eq '교육'}"> checked="checked"</c:if>/> 교육 &nbsp;
						<input type="radio" name="gubun" value="행사" <c:if test="${result.gubun eq '행사'}"> checked="checked"</c:if>/> 행사 &nbsp;
						<input type="radio" name="gubun" value="세미나" <c:if test="${result.gubun eq '세미나'}"> checked="checked"</c:if>/> 세미나 &nbsp;
						<input type="radio" name="gubun" value="공모전" <c:if test="${result.gubun eq '공모전'}"> checked="checked"</c:if>/> 공모전 &nbsp;
						<input type="radio" name="gubun" value="이벤트" <c:if test="${result.gubun eq '이벤트'}"> checked="checked"</c:if>/> 이벤트
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="">기간</label><span title="필수입력">*</span></th>
					<td>
						<input name="sDate" id="sDate" type="text" class="search-date-picker" style="width:140px" value="<c:out value="${result.sDate}" />" readOnly />
						<div></div>
						<input name="eDate" id="eDate" type="text" class="search-date-picker" style="width:140px" value="<c:out value="${result.eDate}" />" readOnly />
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="">내용</label><span title="필수입력">*</span></th>
					<td>
						<textarea name="contents" class="textarea-board" cols="50" rows="2" title="내용 입력"><c:out value="${result.contents}" /></textarea>
					</td>
				</tr>
			</tbody>
		</table>
