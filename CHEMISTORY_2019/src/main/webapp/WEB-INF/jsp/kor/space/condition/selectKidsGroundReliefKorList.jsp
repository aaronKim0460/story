...
doDetail = function(sn){
	...
	var newPop = document.listform;
	var url = "<c:url value='/kor/space/conditionReliefDetail.do'/>";
	 
	var status = "width=712, height=384, top=30,left=60"; 
	window.open("","listform",status);
	
	newPop.action =url; 
	newPop.method="post";
	newPop.target="listform";
	newPop.submit();	
...
function linkPage(pageNo){
    ...
    document.listform.action = "<c:url value='/kor/space/conditionRelief.do'/>";
					...
					<fieldset>
						...
						<p>
							...
							<select id="searchAreaCl" name="searchAreaCl" class="select_list">
								...
								<option value="강원" <c:if test="${krvo.searchAreaCl == '강원'}">selected="selected"</c:if>  >강원</option>
								<option value="경기" <c:if test="${krvo.searchAreaCl == '경기'}">selected="selected"</c:if>  >경기</option>
								<option value="경남" <c:if test="${krvo.searchAreaCl == '경남'}">selected="selected"</c:if>  >경남</option>
								<option value="경북" <c:if test="${krvo.searchAreaCl == '경북'}">selected="selected"</c:if>  >경북</option>
								<option value="광주" <c:if test="${krvo.searchAreaCl == '광주'}">selected="selected"</c:if>  >광주</option>
								<option value="대구" <c:if test="${krvo.searchAreaCl == '대구'}">selected="selected"</c:if>  >대구</option>			
								<option value="대전" <c:if test="${krvo.searchAreaCl == '대전'}">selected="selected"</c:if>  >대전</option>			
								<option value="부산" <c:if test="${krvo.searchAreaCl == '부산'}">selected="selected"</c:if>  >부산</option>			
								<option value="서울" <c:if test="${krvo.searchAreaCl == '서울'}">selected="selected"</c:if>  >서울</option>			
								<option value="세종" <c:if test="${krvo.searchAreaCl == '세종'}">selected="selected"</c:if>  >세종</option>			
								<option value="울산" <c:if test="${krvo.searchAreaCl == '울산'}">selected="selected"</c:if>  >울산</option>			
								<option value="인천" <c:if test="${krvo.searchAreaCl == '인천'}">selected="selected"</c:if>  >인천</option>			
								<option value="전남" <c:if test="${krvo.searchAreaCl == '전남'}">selected="selected"</c:if>  >전남</option>			
								<option value="전북" <c:if test="${krvo.searchAreaCl == '전북'}">selected="selected"</c:if>  >전북</option>			
								<option value="제주" <c:if test="${krvo.searchAreaCl == '제주'}">selected="selected"</c:if>  >제주</option>			
								<option value="충남" <c:if test="${krvo.searchAreaCl == '충남'}">selected="selected"</c:if>  >충남</option>			
								<option value="충북" <c:if test="${krvo.searchAreaCl == '충북'}">selected="selected"</c:if>  >충북</option>
							</select>
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				...
				<tbody id="body_list" class="t_center list">
					...
					<tr>
						<td><strong style="color:#18a6e8;">[
							...
							<!-- <a href="#none" onclick="doDetail('${kvo.crtfcSn}')"><c:out value='${kvo.fcltyNm}'/></a>-->
							<c:out value='${kvo.fcltyNm}'/>
							<span style="font-size:12px; color:#666;"><c:out value="${kvo.crtfcBgnde}"/> ~ <c:out value="${kvo.crtfcEndde}"/></span>
							</strong>
							<br/><span style="font-size:12px; color:#666;"><c:out value="${kvo.locplc}"/></span>
						</td>		
						<td><c:out value="${kvo.crtfcNo}"/></td>			
					</tr>
					</c:forEach>
		
					<c:if test="${fn:length(kvoList) == 0}">
						<tr>
							<td colspan="3" class="no_data" align="center">작성된 게시물이 없습니다.</td>
						</tr>
					</c:if>
