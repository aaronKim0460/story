...
<!--
	...
	function submitFrmSchedule(yearStr, monthStr, dayStr){
		...
		document.frm.action = "<c:url value='/cop/com/selectNierScheduleList.do'/>";
		document.frm.submit();
	}
	
	<c:if test="${fn:length(tilesPrefix) < 1 && empty param.moveSite}">
	function egov_scheduleInsert(){
	...
	function fn_egov_schdulInqire(stRegdt, stSeq, deptCd, sectCd){
		...
		document.frm.action = "<c:url value='/cop/com/selectSchedule.do'/>";
		document.frm.submit();
	}
//-->
</script>
...
<!-- 										
	...
	<a href="javascript:egov_scheduleInsert();" class="btn seablue">등록</a>
	</c:if>	
</div>
</form>
<!-- //contents -->