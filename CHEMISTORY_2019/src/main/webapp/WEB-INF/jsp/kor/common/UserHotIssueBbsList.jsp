...
<script type="text/javascript">
	...
	// 상세화면으로 이동
	function fn_egov_inqire_notice(nttId, bbsId, keyword) {
		$('#nttId').val(nttId);
		$('#bbsId').val(bbsId);
		$('#nttSj').val(keyword);
...
<div id="contents">
	<div class="tableBtn clearfix margin_t_25">
		...
		<input type="hidden" name="nttSj" id="nttSj" value="" />
			...
			<div class="newsletter">
				<ul class="clearfix">
					...
					<li>
						...
						<a href="#/cop/bbs/selectBoardArticle.do?nttId=<c:out value='${result.nttId}'/>&amp;bbsId=<c:out value='${result.bbsId}'/>&amp;menuNo=<c:out value='${menuManageVO.menuNo}'/>" title="<c:out value="${result.nttSj}"/>" 
						   onclick="javascript:fn_egov_inqire_notice('<c:out value='${result.nttId}'/>','<c:out value='${result.bbsId}'/>','<c:out value='${result.nttSj}'/>'); return false;">
						...
						<c:choose>
							...
							<div class="newText" style="padding-bottom:10px;">
