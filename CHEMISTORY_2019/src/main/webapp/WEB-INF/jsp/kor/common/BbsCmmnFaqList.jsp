			...
			<div class="btn_right">
				<div class="board_search">
					<input type="hidden" name="bbsId" id="bbsId"
						...
						type="hidden" name="searchCl1DetailCode" id="searchCl1DetailCode" value="<c:out value='${searchVO.searchCl1DetailCode}'/>" />
					<fieldset>
	...
	<table class="bbs_default list">
		...
		<tbody id="body_list" class="t_center list">
			<c:forEach var="result" items="${resultList}" varStatus="status">
					...
					onclick="javascript:fn_egov_inqire_notice('<c:out value='${result.nttId}'/>','<c:out value='${result.bbsId}'/>'); return false;">
						<c:choose>
							<c:when test="${result.isExpired=='Y' || result.useAt == 'N'}">
								...
								<c:if
									test="${userAuthority != null && result.publicNuriTy != '0' && result.publicNuriTy != ''}">
									<img
				...
				<c:if test="${userAuthority != null }">
					<td class="board-default-td-name" onclick="javascript:fn_egov_inqire_faq('<c:out value='${result.nttId}'/>','<c:out value='${result.bbsId}'/>','<c:out value="${paginationInfo.totalRecordCount - ((searchVO.pageIndex -1)* searchVO.pageSize + status.index)}" />'); return false;"><c:choose>
