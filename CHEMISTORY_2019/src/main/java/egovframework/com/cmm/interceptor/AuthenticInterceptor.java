...
public class AuthenticInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		String requestURI = request.getServletPath(); //요청 URI
		...
		if(loginVO != null){
			...
			for(Iterator<String> it = this.permittedURL.iterator(); it.hasNext();){
				...
				if ("/validator.do".equals(requestURI)) {
					isPermittedURL = true;
				}
			}
			
			if(!isPermittedURL){
				ModelAndView modelAndView = new ModelAndView("redirect:/uat/uia/actionMoveLogin.do");			
