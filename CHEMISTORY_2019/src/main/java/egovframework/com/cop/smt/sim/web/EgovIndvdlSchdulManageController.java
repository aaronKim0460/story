...
public class EgovIndvdlSchdulManageController {
			...
			****************************************************************** */
    		...
    		if(!files.isEmpty()){
    			...
    			if("N".equals(atchFileAt)){
    				List<FileVO> _result = fileUtil.parseFileInf(files, "DSCH_", 0, _atchFileId, "", thumbVO);
    				...
    				List<FileVO> _result = fileUtil.parseFileInf(files, "DSCH_", _cnt, _atchFileId, "", thumbVO);
    ...
    throws Exception {
        ...
        if("save".equals(sCmd)){
    		...
    		if(!files.isEmpty()){
    		 _result = fileUtil.parseFileInf(files, "DSCH_", 0, "", "", thumbVO);
