...
public class EgovBBSUseInfoManageController {
	...
	bdUseVO.setUseAt(bdUseVO.getSearchUseAt());
	...
	if ("SYSTEM_DEFAULT_BOARD".equals(vo.getTrgetId())) {
	    ...
	    } else  if (vo.getBbsTyCode().equals("BBST13")) {
	    	vo.setProvdUrl(request.getContextPath()+ "/cop/bbs/selectExpBoardList.do?bbsSubOpt=" + vo.getBbsSubOpt());
	    }else {
	    	if(vo.getLoginReqAt().equals("Y")){
