...
public class EgovBBSCommentController {
    ...
    public String insertComment(@ModelAttribute("searchVO") CommentVO commentVO, @ModelAttribute("comment") Comment comment, 
		...
		if (isAuthenticated) {
		    ...
		    if(comment.getCommentPassword().equals("dummy")){
		    	comment.setCommentPassword("");	// dummy 오류 수정 (익명이 아닌 경우 validator 처리를 위해 dummy로 지정됨)
		    }
	
		    if(comment.getWrterNm().equals("")){ //게시물의 wrterNm이 없는 경우 로그인한 계정의 이름으로 저장
		    	comment.setWrterNm(user.getName()); //게시물 통계 집계를 위해 등록자 Name 저장
		    }
	
		    comment.setCommentCn(unscript(comment.getCommentCn()));	// XSS 방지
		    
		    System.out.println("getWrterNm:"+comment.getWrterNm());
		    System.out.println("getCommentCn:"+comment.getCommentCn());
		    
		    if(!comment.getCommentCn().equals("")) {
