...
public class EgovLoginController {
   ...
   private EgovMenuManageService menuManageService;
    ...
    public String loginUsrView(@ModelAttribute("loginVO") LoginVO loginVO,
    	...
    	if(loginVO.getDn() == null || loginVO.getDn().equals("")){
    	...
    	HttpServletRequest ipData = ((ServletRequestAttributes)RequestContextHolder.getRequestAttributes()).getRequest();
    	if(!myIp.equals("127.0.0.1")){
    		myIp = ipData.getHeader("Proxy-Client-IP");
    	}
    	
//    	System.out.println("!!! myIp == "+myIp);
    	
    	loginVO.setIp(myIp);
//    	pass = loginService.ipChk(loginVO);
...
//    	System.out.println("!!! pass== "+pass);
    ...
    /**
     * 로그인 화면으로 들어간다 (공인인증서 로그인)
     * @param vo - 로그인후 이동할 URL이 담긴 LoginVO
     * @return 로그인 페이지
     * @exception Exception
     */
    @IncludedInfo(name="로그인", listUrl="/uat/uia/egovLoginUsrPKI.do", order = 10, gid = 10)
    @RequestMapping(value="/uat/uia/egovLoginUsrPKI.do")
    public String loginUsrViewPKI(@ModelAttribute("loginVO") LoginVO loginVO,
    		HttpServletRequest request,
    		HttpServletResponse response,
    		ModelMap model) 
    				throws Exception {
    	 ...
    	 */
    	return "egovframework/com/uat/uia/EgovCmsLoginUsrPKI"; 
    }
	...
	public String actionMain(ModelMap model) 
    	...
    	if(!isAuthenticated) {
//    		model.addAttribute("message", egovMessageSource.getMessage("fail.common.login"));
        	//return "egovframework/com/uat/uia/EgovCmsLoginUsr";//2015.11.02 수정 ikho
        	return "redirect:/uat/uia/egovLoginUsr.do?dn=s"; //2017.06.16
    	}
		...
		LOG.debug(user.getSiteType() + ":" + user.getId());
		...
		if (main_page != null && !main_page.equals("")) {
			LOG.debug("Globals.MAIN_PAGE > " +  main_page);
			// 3-1. 설정된 메인화면이 있는 경우
			return main_page;
			...
			if (user.getUserSe().equals("USR")) {
				LOG.debug("Globals.MAIN_PAGE > egovframework/com/EgovMainView");
	    		//return "egovframework/com/EgovMainView";
				return "common/ComAdmActionLogin";
	    	} else {
	    		LOG.debug("Globals.MAIN_PAGE > egovframework/com/EgovMainViewG");
	...
	public String actionLogout(HttpServletRequest request, ModelMap model) 
    	...
    	return "forward:/uat/uia/egovLoginUsr.do?dn=s";
    ...
    @RequestMapping(value = "/uat/uia/actionMoveLogin.do")
    public String actionMoveLogin(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
    	return "egovframework/com/uat/uia/VtekFrameLogoutUsr";
    }
}